...
import { SpringOptions, motion, animate, transform, useMotionValue, useSpring } from 'framer-motion';
    ...
    useEffect(() => {
        const manageMouseMove = (e: MouseEvent) => {
            ...
            if (isHovered && hoveredElementRef.current) {
                const { top, left, width, height } = hoveredElementRef.current.getBoundingClientRect();
                const center = { x: left + width / 2, y: top + height / 2 };
                const r = { x: clientX - center.x, y: clientY - center.y };

                const distance = getDistanceFromRect(hoveredElementRef.current.getBoundingClientRect(), clientX, clientY);

                const newScaleX = transform(distance, [0, UNSTICK_THRESHOLD_DISTANCE], [1, 1.2]);
                const newScaleY = transform(distance, [0, UNSTICK_THRESHOLD_DISTANCE], [1, 0.8]);
                ...
                animate(hoveredElementRef.current, { x: r.x * 0.1, y: r.y * 0.1 }, { duration: 0 });

                if (distance > UNSTICK_THRESHOLD_DISTANCE) {
                    setIsHovered(false);
                    animate(hoveredElementRef.current, { x: 0, y: 0 }, { type: 'spring', stiffness: 300, damping: 10, mass: 1 });
    ...
    return (
        <>
            <motion.div
                ...
                className="w-5 h-5 fixed bg-black rounded-full pointer-events-none z-50"
                animate={{
                    width: cursorSize,
                    height: cursorSize,
                }}
